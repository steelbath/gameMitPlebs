import numpy as np
import pygame as pg
from classes import * 
import GameFunctions as gf
from class1 import *
import sys

Clock = pg.time.Clock()

#saving mobs as rectangles in mobs_rect and the mobs object in mobs
def mobs_create():
    mobs=[]
    mobs_rect=[]
    enemy = pg.image.load("enemy sprite slurp.png")

    for i in np.arange(5):
        mobs.append(Enemy(enemy,[np.random.randint(-10,10), np.random.randint(-10,10)],[300,300]))
        mobs_rect.append(mobs[i].rect)


    return np.array([mobs,mobs_rect])




def main():
    pg.init()
    GS = GameSettings()

    #getting the mobs objects from the function
    mobs = mobs_create()[0]
    
    pg.display.init()
    screen = pg.display.set_mode((1920,1080),flags = pg.HWSURFACE|pg.FULLSCREEN)

    print(mobs_create()[1])
    mobs_center = [0]*5
    collide_array = np.zeros((5))
    player = pg.image.load("Spielfigur.png").convert()
    background = pg.image.load("background.png").convert()
    projectile = pg.image.load("red_square.png").convert()
    
    pg.mouse.set_visible(True)
    running = True
    #getting the Player object
    #Player_0 = Player(player, 5, screen, background) 
    Player1 = Player(screen, pg.image.load("Spielfigur.png").convert(), (500,500),(0,0))
    
    while running:
        #print(Player1.projectiles)
        check_events_return = gf.check_events(Player1)
        screen.blit(background, (0,0))
        #adding (-5) to the y-Coordinate the projectiles to "move" them 
        projectiles, event_number, running = check_events_return[0], check_events_return[1], check_events_return[2]
        
        
        for m in np.arange(100):
            #only consider the projectiles which are not zero at the x-coordinate to
            #prevent the zeros to get blitted
            if projectiles[m,0] > 0:
                screen.blit(projectile, (projectiles[m,0], projectiles[m,1]))
            #delete the projectiles which are actually projectiles (x > 0) and are out of the screen (y < 0)
            if projectiles[m,1] < 0 and projectiles[m,0] > 0:
                projectiles[m]=np.zeros((2))
        projectiles -= np.array([0,5])   
            
        #blit the mobs
        for idx, m in enumerate(mobs):   
            mobs_center[idx] = m.spieler_zerlegen()
            mobs_zeros = np.array([pg.Rect(0,0,0,0)]*5)
            #print(mobs_center)
            #mobs_center[m] = mobs_position[m]
            screen.blit(m.image,m.rect) 
        
        for idx, k in enumerate(mobs):
            mobs_zeros[idx] = mobs_center[idx]
            collide_array[idx] = pg.Rect(mobs_center[idx]).collidelist(mobs_center-mobs_zeros)
            mobs_zeros[idx] = pg.Rect(0,0,0,0)
          
            if not collide_array[idx] == -1:
                k.collision = True
          

               
            





        #blit the players position and the movement
        
        Player1.checkKeys()
        Player1.update()
        screen.blit(player, Player1.pos)
        pg.display.update()
        Clock.tick(120)


main()